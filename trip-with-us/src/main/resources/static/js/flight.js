...
function showFlightDetail(flight) {
  ...
  document.getElementById('priceFreezeBox').classList.add('hidden');
  loadFlightReviews();
  loadTrackedFlights();
...
function proceedFromFlightDetail() {
  ...
  document.getElementById('seatFlightTitle').textContent = selectedFlight.airline + ' ' + selectedFlight.flightNumber + ' — ' + selectedFlight.fromCity + ' to ' + selectedFlight.toCity;
  document.getElementById('seatFlightMeta').textContent = selectedFlight.travelDate + ' · Departs ' + selectedFlight.departureTime;
  updateSteps('seats');
  loadFlightSeats();
}
...
async function loadFlightSeats() {
  ...
  try {
    const seats = await get('/flights/' + selectedFlight.id + '/seats');
    seatMap.innerHTML = '';
    seats.forEach(s => {
    ...
    seatMap.innerHTML = '<p class="error-text" style="display:block;">Failed to load seat map.</p>';
  }
  updateSeatSummary();
}
...
function updateSeatSummary() {
  document.getElementById('selectedSeatsLabel').textContent =
    selectedSeats.length + ' seat(s) selected: ' + (selectedSeats.map(s => s.seatNumber).join(', ') || 'none');
...
async function refreshLiveStatus() {
  ...
  showStatusPushNotification(selectedFlight);
}

/* ============================================================
   Push Notification Simulation — mimics a device push when a
   tracked flight's status/ETA changes.
   ============================================================ */
let _lastNotifiedStatus = null;

function showStatusPushNotification(flight) {
  try {
    if (!flight || !flight.liveStatus) return;
    const key = flight.liveStatus + '|' + (flight.estimatedArrival || flight.arrivalTime);
    if (key === _lastNotifiedStatus) return;
    _lastNotifiedStatus = key;

    const overlay = document.createElement('div');
    overlay.className = 'push-toast';
    const title = '✈️ ' + flight.airline + ' ' + flight.flightNumber;
    let body = 'Status: ' + flight.liveStatus;
    if (flight.delayReason) body += ' — ' + flight.delayReason;
    if (flight.estimatedArrival) body += ' | Est. arrival ' + flight.estimatedArrival;
...
/* ============================================================
   Price Freeze — lock today's fare for 30 minutes
   ============================================================ */
async function freezePrice() {
  if (!selectedFlight) return;
  const box = document.getElementById('priceFreezeBox');
  const msg = document.getElementById('freezeMsg');
  // Check if there is already an active freeze
  try {
    const active = await get('/pricing/freeze/active?type=FLIGHT&targetId=' + selectedFlight.id);
    if (active != null) {
      msg.style.display = 'block';
      msg.style.color = 'var(--success)';
      msg.textContent = '✅ Active price freeze found: ' + fmtCurrency(active) + ' — valid for this flight.';
...
async function confirmFreezePrice() {
  if (!selectedFlight) return;
  const msg = document.getElementById('freezeMsg');
  try {
    const freeze = await post('/pricing/freeze', {
      type: 'FLIGHT',
      targetId: selectedFlight.id,
      price: selectedFlight.currentPrice,
      minutes: 30
    });
    msg.style.display = 'block';
    msg.style.color = 'var(--success)';
    msg.textContent = '✅ Price frozen at ' + fmtCurrency(freeze.frozenPrice) + ' for 30 minutes (until ' +
      new Date(freeze.expiresAt).toLocaleTimeString() + ').';
  } catch (err) {
    msg.style.display = 'block';
...
/* ============================================================
   Tracked flights — load and display user's tracked flights
   ============================================================ */
async function loadTrackedFlights() {
  try {
    const tracked = await get('/flights/tracked');
    const box = document.getElementById('trackedFlightsBox');
    const list = document.getElementById('trackedFlightsList');
    if (!box || !list) return;
    if (!tracked.length) {
    ...
    box.classList.remove('hidden');
    list.innerHTML = tracked.map(function(t) {
      const fid = t.flightId;
      return '<div style="background:var(--bg); border-radius:8px; padding:10px 14px; margin-top:8px; font-size:13px;">' +
        'Flight #' + fid + ' — tracking live <span class="muted">(refresh status to see updates)</span>' +
      '</div>';
    }).join('');
...
function getFlightBase() {
  const seatCount = Math.max(1, selectedSeats.length || 1);
  return (selectedFlight.currentPrice * seatCount) + seatPriceExtra;
}

function refreshSummary() {
  const base = getFlightBase();
...
async function applyCoupon() {
  ...
  try {
    ...
    discountAmount = getFlightBase() * (coupon.discountPercent / 100);
...
async function confirmBooking(paymentInfo) {
  ...
  try {
    const booking = await post('/bookings', {
      ...
      passengerCount: selectedSeats.length || 1,
    ...
    updateSteps('confirm');
    const seatLabel = selectedSeats.length ? 'Seats: ' + selectedSeats.map(s => s.seatNumber).join(', ') + ' \u00b7 ' : '';
    document.getElementById('confirmMsg').textContent = 'Booking #' + booking.id + ' confirmed for ' + fmtCurrency(booking.finalAmount) + '. ' + seatLabel;
    document.getElementById('paymentReceipt').innerHTML =
      '<strong>Payment Receipt</strong><br>' +
      (seatLabel ? seatLabel + '<br>' : '') +
